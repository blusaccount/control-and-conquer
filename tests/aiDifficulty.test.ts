...
test("decision cadence bounds match OpenFront's per-difficulty nextInt ranges", () => {
  // Every controller rolls its cadence from these bounds; pin them here so a
  // rebalance is a deliberate act. Impossible reacts about twice as often as
  // Easy (30–50 vs 65–100 ticks); tribes always roll nextInt(40, 80).
  assert.deepEqual(NATION_DECISION_TICKS.easy, [65, 100]);
  assert.deepEqual(NATION_DECISION_TICKS.medium, [55, 70]);
  assert.deepEqual(NATION_DECISION_TICKS.hard, [45, 60]);
  assert.deepEqual(NATION_DECISION_TICKS.impossible, [30, 50]);
  assert.deepEqual(BOT_DECISION_TICKS, [40, 80]);
  assert.ok(NATION_DECISION_TICKS.impossible[1] * 2 <= NATION_DECISION_TICKS.easy[1]);
});

test("buildFieldConfigs is bot-heavy and carries difficulty + per-seat seeds", () => {
  ...
  for (const [i, c] of configs.entries()) {
    assert.equal(c.difficulty, "hard", "every seat carries the match difficulty");
    assert.equal(c.seed, i, "the seat index seeds the controller's PRNG");
  }
});
